...
object AvroTest {
  ...
  def createSampleDataFile[T <: SpecificRecord](avroType: AvroRecordType[T], records: Iterable[T]) : File = {
    ...
    val writer = avroType.createAvroRecordAppender(file, CodecFactoryProvider.codecFactory())
    try {
  ...
  def createProcessedDataFile[T <: SpecificRecord](integration: ProcessedDataIntegration[T], task: DataIngestionTask) : File = {
    val appender  = integration.avroType().createAvroRecordAppender(newTempTestFile(), CodecFactoryProvider.codecFactory())
