 ...
 */
abstract class WorkerActorTests(val config: WorkerConfig) extends TestKit(ActorSystem("TestActorSystem", config.underlying)) with WordSpecLike with BeforeAndAfterAll with Matchers with ImplicitSender {
