...
/**
  ...
  */
trait DelegatingWorkResponseHandler extends WorkResponseHandler {
  ...
  override final def handleWorkCompleted(response: WorkerResponse) : Unit = response match {
  ...
  override final def handleWorkExpired(expired: WorkerInput) : Unit = expired match {
  ...
  override final def handleWorkFailed(failed: FailureResponse) : Unit = failed.input match {
    case i: ValidateIntegrationRequest  =>
